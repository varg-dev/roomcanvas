/**
 * Source: https://github.com/cginternals/webgl-operate/blob/aafb4f68f43659dba29a0a63974dd214c057fd5c/demos/point-cloud/benchmark.ts
 */
...
export class Benchmark {
    ...
    initialize(
        cycles: number,
        framesForWarmup: number,
        framesPerRun: number,
        framePrepare: FramePrepareCallback,
        runsFinished: RunsFinishedCallback,
    ): void {
        if (this._running) {
    ...
    frame(): void {
        ...
        if (frames % this._framesPerCycle === 0 && cycle > 0) {
            ...
            console.log(
                ' --  cycle: ' +
                    cycle.toString().padStart(2, '0') +
                    ', frame time [ms]: ' +
                    this._results[cycle - 1].toFixed(4).padStart(9, '0') + 
                    ', fps: ' + 
                    (1000 / this._results[cycle - 1]).toFixed(4).padStart(9, '0'),
            );
        }
